...
2. 双分支
   ...
   说明：条件表达式为 true 时，执行 代码块1 中的代码，否则就执行 代码块2 中的代码
...
3. for 循环的第三种使用方式
   ...
   上面的写法 是一个 无限循环 => 死循环，通常要配合 break 语句使用

对应 for-range 遍历方式而言，是按照字符方式遍历，因此如果有字符串有中文，也能执行

跳出 for 循环： break ，goto，return，其中 break 后面可以加关键字，表示可以跳出哪一层循环，一般程序中不使用 goto ，return 使用在函数中，可以理解成 终止函数，终止程序。
